...
In this example we consider a paternity case where we have a mixed DNA profile of a mother and child, in addition to reference profiles from the mother and the alleged father. <!-- The example was used in Dorum et al. (2016).-->
...
If there are multiple replicates, the next replicate profile must have a different sample name (e.g. "relMix2") and follow directly after the first replicate.

####1b. Reference profiles

Table 2 shows the format of the file containing the reference profiles (all reference profiles have to be in the same file). The first line should correspond to the column names seen here. The different reference profiles follow directly after each other, and each profile should have a specific sample name. In our example there are two reference profiles, "Mother" and "Father". **Note:** as long as the built-in pedigrees are used, the reference profiles should be named either "Father", "Mother" or "Child" because these are the only individual IDs that are defined in these pedigrees. If custom pedigrees are provided, any sample name can be used (see example 2 below). Only the first two markers and the last one are shown for each profile.
...
####1c. Database
<!-- 
Two figures of equal size next to each other
...
<!-- Two figures of unequal size next to each other -->
<!-- 
```{r, out.width = '35%', fig.align='center', echo=FALSE, out.extra='style="float:left"'}
knitr::include_graphics("database_crop.PNG")
```{r, out.width = '55%', fig.align='center', echo=FALSE, par(mar=c(5,8,4,2)+0.1), out.extra='style="float:right"'}
knitr::include_graphics("database_options.png")
```
...
###6. Compute LR
Finally, we are ready to click the 'Compute LR' button. The result appears in a new pop-up window (Figure 7). The chosen parameters are displayed on the left, and the total LR followed by the LRs per marker are displayed on the right. By clicking "Save results to file", the LRs together with the mixture profiles, reference profiles and the parameters will be saved to file.
...
The first step in RelMix is to import data from tab delimited .txt-files. Examples will be given for the three types of files that are needed: mixture profile, reference profiles and allele frequencies.

####Mixture profile

The following table shows an example of the format of a mixture file. The first line of the file contains the column names and must correspond with the names given here, i.e. 'SampleName', 'Marker', etc. The file may be expanded with columns 'Allele4', 'Allele5'and so on if there are more than three unique alleles in the mixture.
```{r, echo=FALSE, results='asis'}
M <- read.table("mixture.txt",sep="\t",header=TRUE)
...
Table 2 shows the format of the file containing reference profiles. If there are more than one known profiles, the next profile should follow directly after the first one, and each profile is indicated with a specific sample name. In our example there are two known profiles, "Mother" and "Father". \textit{Important:} The names of the individuals in a paternity case must be "Father", "Mother" and "Child", unless custom pedigrees are provided (see below), in which case the sample names must correspond with the individuals ID in the provided pedigree file.
```{r, echo=FALSE, results='asis'}
G <- read.table("references.txt",sep="\t",header=TRUE)
...
```{r, out.width = '35%', fig.align='center', echo=FALSE, out.extra='style="float:left"'}
knitr::include_graphics("database_crop.PNG")
```{r, out.width = '55%', fig.align='center', echo=FALSE, out.extra='style="float:right"'}
knitr::include_graphics("database_options.png")
```
<div style="clear: both;"></div>
-->
